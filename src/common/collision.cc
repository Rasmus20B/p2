...
export std::vector<line> getAxis(const Rectangle& r, const float radian) {
  const Vector2 ox = { 1, 0 };
  const Vector2 oy = { 0, 1 };

  const Vector2 rx = rotate(ox, radian);
  const Vector2 ry = rotate(oy, radian);
...
export std::array<Vector2, 4> getCorners(const Rectangle& r, const float radian) {
...
// }
//
export bool checkContains(std::array<Vector2, 4> cs1, std::array<Vector2, 4> cs2) {
  for(int i = 0; i < 4; ++i) {
    Vector2 edge = minus(cs1[(i+1) % 4], cs1[i]);
    ...
    for(int j = 0; j < 4; ++j) {
      ...
      float proj1 = dot(cs1[j], {axis});
      float proj2 = dot(cs2[j], {axis});
...
// #endif
//
export bool CheckCollisionRecsAngle(const Rectangle& r1, const float a1, const Rectangle& r2, const float a2) {
